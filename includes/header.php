

<nav class="navbar navbar-inverse fixed-top" style="margin-top: 5px;">
      <div class="container">
        <div class="navbar-header">
          <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar" style="background-color:#f0ad4e;">
            <span class="sr-only">Toggle navigation</span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
          <a class="navbar-brand" href="#" style="color: #f0ad4e; font-size: 25px;">Web Series Hub</a>
        </div>
        <div id="navbar" class="collapse navbar-collapse" style="border-color: #3c5984; width: 100%;">
          <ul class="nav navbar-nav" style="width: auto; float: right; ">
            <li style="float:right;"><a href="../html/login.php"  style="color: #f0ad4e; font-size: 20px;"><span class="glyphicon glyphicon-log-in"></span> Sign in</a></li>

          </ul>
        </div><!--/.nav-collapse -->
      </div>
    </nav>